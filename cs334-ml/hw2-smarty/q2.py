...
def holdout(model, xFeat, y, testSize):
    ...
    xTrain, xTest, yTrain, yTest = train_test_split(
        xFeat, y, test_size=testSize, random_state=0)
...
def kfold_cv(model, xFeat, y, k):
    ...
    for i_train, i_test in k_fold.split(xFeat):
        xTrain, xTest, yTrain, yTest = xFeat.iloc[i_train], xFeat.iloc[i_test], y.iloc[i_train], y.iloc[i_test]
        train, test = sktree_train_test(model, xTrain, yTrain, xTest, yTest)
        trains.append(train)
        tests.append(test)

    trainAuc, testAuc = np.mean(trains), np.mean(tests)
...
def mc_cv(model, xFeat, y, testSize, s):
    ...
    for _ in range(s):
        xTrain, xTest, yTrain, yTest = train_test_split(
            *shuffle(xFeat, y), random_state=0, test_size=testSize)
        train, test = sktree_train_test(model, xTrain, yTrain, xTest, yTest)
        trains.append(train)
        tests.append(test)
