...
def standard_scale(xTrain, xTest):
    ...
    return normalize(xTrain), normalize(xTest)
...
def minmax_range(xTrain, xTest):
    ...
    def minmax_norm(df: np.ndarray):
        return (df - df.min()) / (df.max() - df.min())

    return minmax_norm(xTrain), minmax_norm(xTest)


def add_irr_feature(xTrain: pd.DataFrame, xTest):
    """
    ...
    """
    xTrain['BadCol'] = np.random.normal(size=xTrain.shape[0])
    xTrain['BadCol2'] = np.random.normal(size=xTrain.shape[0])
    xTest['BadCol'] = np.random.normal(size=xTest.shape[0])
    xTest['BadCol2'] = np.random.normal(size=xTest.shape[0])
    return xTrain, xTest
...
def knn_train_test(k, xTrain, yTrain, xTest, yTest):
    ...
    Parameters
    ----------
    train_accuracy: Returns train accuracy instead of test accuracy if True
    k : int
