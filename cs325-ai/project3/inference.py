    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            new_positions = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            for new_pos, prob in new_positions.items():
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            ...
            counter = util.Counter(
                {p: emissionModel[util.manhattanDistance(pacmanPosition, p)] * belief[p] for p in self.legalPositions})

            if counter.totalCount() != 0:
                self.particles = [util.sample(counter) for _ in range(self.numParticles)]
            else:
                # Special Case 2
                self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[i]))) for i in range(len(self.particles))]
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Divide by number of particles to normalize, don't forget to include the jail!
        return util.Counter({pos: float(self.particles.count(pos)) / self.numParticles for pos in
                             [self.getJailPosition()] + self.legalPositions})
