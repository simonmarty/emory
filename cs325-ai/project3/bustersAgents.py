...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i, beliefs
                                            ...
                                            if livingGhosts[i + 1]]
        "*** YOUR CODE HERE ***"
        possible_positions = [ghostPosDist.argMax() for ghostPosDist in livingGhostPositionDistributions]

        # Note that in Python, tuples sort by their first item
        distance, goal = min([(self.distancer.getDistance(pacmanPosition, position), position)
                              for position in possible_positions])

        best_distance, best_action = min(
            [(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), goal), action) for action in
             legal])
        return best_action
